...
void process_start()
{
    ...
    if (configData->externalDataLoadMethod == CONFIG_externalDataLoadMethod_ByPointerReplacement)
        sourceData(&threadInfo, fullContext);
    else
...
efi_main (EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    if (configData->externalDataLoadMethod == CONFIG_externalDataLoadMethod_ByPointerReplacement)
    {
        print(L"[INFO] Attempting to load external data by Pointer Replacement\n");
        unsigned char *sourceDataAsBytes = sourceData;
        unsigned long addrSignature = configData->externalDataLoadReplacementPointerValue;
        unsigned long addrActual = extData;
        unsigned long currentSignature = 0;
        unsigned long pendingSkips = sizeof(unsigned long) - 1;
        unsigned long ptrsReplaced = 0;
        
        for (int i = 0; i < configData->mainBytesSize; i++)
        {
            if (pendingSkips > 0)
            {
                pendingSkips --;
            ...
            currentSignature = *(unsigned long *)&sourceDataAsBytes[i-7];

            if (addrSignature == currentSignature)
            {
                pendingSkips = sizeof(unsigned long) - 1;
                ptrsReplaced ++;
                *(unsigned long *)&sourceDataAsBytes[i-7] = addrActual;
            }
        }
        print(L"[INFO] Completed load of external data by Pointer Replacement\n");
        print(L"[INFO] Pointer Replacement replaced %d pointers\n", ptrsReplaced);
        if (ptrsReplaced != configData->externalDataLoadReplacementPointerCount){print(L"[ERROR] Expected to replace %d pointers. But replaced %d !\n", configData->externalDataLoadReplacementPointerCount, ptrsReplaced); while(true);};
    }
    ...
    efi_malloc(&fullContext, sizeof(System));
        efi_malloc(&fullContext->mem, sizeof(struct System_Mem));
            ...
            fullContext->mem->memset = efi_memset;
            fullContext->mem->memcpy = efi_memcpy;
        efi_malloc(&fullContext->io, sizeof(struct System_IO));
            fullContext->io->log = print;
            fullContext->io->scan = scan;
            fullContext->io->output = output;
            fullContext->io->input = input;
        efi_malloc(&fullContext->misc, sizeof(struct System_Misc));
            fullContext->misc->stall = efi_stall;
        efi_malloc(&fullContext->efi, sizeof(struct System_Efi));
            fullContext->efi->efiSystemTable = SystemTable;
            fullContext->efi->efiImageHandle = &ImageHandle;
            efi_malloc(&fullContext->efi->efiCallers, sizeof(struct System_Efi_Callers));
                fullContext->efi->efiCallers->call0 = efi_call0;
                fullContext->efi->efiCallers->call1 = efi_call1;
                fullContext->efi->efiCallers->call2 = efi_call2;
                fullContext->efi->efiCallers->call3 = efi_call3;
                fullContext->efi->efiCallers->call4 = efi_call4;
                fullContext->efi->efiCallers->call5 = efi_call5;
                fullContext->efi->efiCallers->call6 = efi_call6;
                fullContext->efi->efiCallers->call7 = efi_call7;
                fullContext->efi->efiCallers->call8 = efi_call8;
                fullContext->efi->efiCallers->call9 = efi_call9;
                fullContext->efi->efiCallers->call10 = efi_call10;
    // Completed load of fullContext
